...
.pr[
```{r, echo=FALSE}
include_graphics("https://aldro61.github.io/microbiome-summer-school-2017/figures/figure.classification.vs.regression.png")
```
...
ridgeg <- ridge_shrink$beta %>%
    ...
    as.data.frame() %>% 
    as_tibble() %>%
    mutate(lambda=rev(ridge_lambda)) %>% 
...
lassog <- lasso_shrink$beta %>%
    ...
    as.data.frame() %>% 
    as_tibble() %>%
    mutate(lambda=rev(lasso_lambda)) %>% 
